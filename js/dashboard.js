...
async function loadDashboard() {
  ...
  for (const novel of novels) {
    ...
    const bailHTML = bails.length > 0
      ? '<div class="section-mini">途中離脱のコメント</div><div class="bail-list">' +
    ...
    const commentHTML = comments.length > 0
      ? '<div class="divider"></div><div class="section-mini">読者コメント</div><div class="bail-list">' +
    ...
    card.innerHTML = `
      ...
      <div class="stats-grid" style="grid-template-columns:repeat(4,1fr)">
        <div class="stat-box">
          <div class="stat-num">${shownCount}</div>
          <div class="stat-label">カード表示回数</div>
        </div>
        <div class="stat-box">
          <div class="stat-num">${chosenCount}</div>
          <div class="stat-label">選ばれた回数</div>
        </div>
        <div class="stat-box">
          <div class="stat-num">${notChosenCount}</div>
          <div class="stat-label">選ばれなかった</div>
        </div>
        <div class="stat-box">
          <div class="stat-num">${chosenPct}%</div>
          <div class="stat-label">選択率</div>
        </div>
      </div>
      ...
      <div class="stats-grid" style="grid-template-columns:repeat(4,1fr);margin-top:8px">
        <div class="stat-box">
          ...
          <div class="stat-label">読了数</div>
        </div>
        <div class="stat-box">
          <div class="stat-num" style="color:#4a9b6f">${good}</div>
          <div class="stat-label">よかった</div>
        </div>
        <div class="stat-box">
          <div class="stat-num" style="color:var(--acc2)">${mid}</div>
          <div class="stat-label">普通</div>
        </div>
        <div class="stat-box">
          <div class="stat-num" style="color:#b85a42">${bad}</div>
          <div class="stat-label">期待外れ</div>
        </div>
      </div>

      ${total > 0 ? `
      <div class="eval-bar" style="margin-top:12px">
